# first author https://pypi.org/project/pntools/
# modified : BoltMaud
#!/usr/bin/python3
# -*- coding_ utf-8 -*-
...
def parse_pnml_file(file):
    ...
    for net_node in root.iter(xmlns+'net'):
        ...
        for arc_node in net_node.iter(xmlns+'arc'):
            ...
            if net.transitions.get(edge.source) != None:
                net.transitions.get(edge.source).postPlaces.append(net.places.get(edge.target))
            else :
                net.transitions.get(edge.target).prePlaces.append(net.places.get(edge.source))
